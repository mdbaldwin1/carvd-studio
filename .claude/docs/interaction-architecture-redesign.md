...
- **Phase §10a Overlay Model foundation is complete** ([ADR-005](./adr/005-overlay-model-derivation.md)): a single pure function `computeOverlayModel` derives every overlay's display data into a typed `OverlayModel` (`snap` / `references` / `dimensions` slots). `SnapAlignmentLines` migrated to a pure prop consumer — it no longer reads from `snapStore`, `projectStore`, or `cameraStore` directly. Workspace subscribes once and passes the model slice down. 8 derivation tests lock in the snap-slot rules + purity. Phase §10b migrates `ReferenceDistanceIndicators` and `MultiSelectionDimensions` to the same pattern.
