...
- **Phase §1a Scene Graph foundation is complete** ([ADR-008](./adr/008-scene-graph-derivation.md)): a memoized `WorkspaceSceneGraph` adapter over `projectStore.parts / groups / groupMembers`. Domain truth still lives in the project store — the adapter is derived, not persisted. `buildWorkspaceSceneGraph` produces a typed node tree with O(1) `findNode`, memoized `descendantPartIds`, `descendantGroupIds`, and `ancestorGroupIds`, defensive duplicate-id / cycle detection. 23+ engine tests cover flat/single-level/nested/disjoint/malformed cases plus the S0–S5 fixtures. Phase §1b migrated consumers including `PartsRenderer`, clipboard, HierarchicalPartsList, PropertiesPanel, keyboard shortcuts, and projectStore hierarchy reads to the adapter. §1c retires the legacy traversal helper once compatibility exports and selection-specific call sites are resolved.
