...
- **Phase §5a Geometry Bundle foundation is complete** ([ADR-009](./adr/009-geometry-bundle-cache.md)): a typed `PartGeometryBundle` per part with bounds (AABB + OBB), render mesh descriptor, hit proxy, snap anchor graph (faces / edges / corners / fraction lines), measurement graph, and collision proxy. `deriveBoxBundle` handles the current box-shaped `Part`; a read-through `GeometryCache` invalidates on dimension or rotation change and returns identical references when version keys match. 28 engine tests cover the version-key rules + per-field bundle correctness + cache lifecycle. Phase §5b migrates the snap engine to read from `bundle.snapGraph`; §5c migrates hit-test rotated-box fallback + measurement + collision. When §6 (Part Shape Model) adds fabrication operations to `Part`, the dispatcher inside `deriveBundle` grows without consumers needing to change.
