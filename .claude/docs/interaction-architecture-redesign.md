...
- **Phase §5a Geometry Bundle foundation is complete** ([ADR-009](./adr/009-geometry-bundle-cache.md)): a typed `PartGeometryBundle` per part with bounds (AABB + OBB), render mesh descriptor, hit proxy, snap anchor graph (faces / edges / corners / fraction lines), measurement graph, and collision proxy. `deriveBoxBundle` handles the current box-shaped `Part`; a read-through `GeometryCache` invalidates on dimension or rotation change and returns identical references when version keys match. 28 engine tests cover the version-key rules + per-field bundle correctness + cache lifecycle. Phase §5c-1 wired the hit-test rotated-box fallback to read from `bundle.hitProxy.localAabb` when a cache is provided. Phase §5b migrates the snap engine; §5c finishes the consumer migrations. When §6 (Part Shape Model) adds fabrication operations to `Part`, the dispatcher inside `deriveBundle` grows without consumers needing to change.
- **Phase §8a Constraint Pipeline foundation is complete** ([ADR-006](./adr/006-constraint-pipeline.md)): a typed `Constraint` interface, an `applyConstraints` pipeline runner, and the two simplest constraints (`groundConstraint`, `stockDimensionConstraint`). Constraints take a `CandidateTransform` (move / resize / rotate variants), produce an adjusted transform plus blockers + warnings. Phase §8b lifts the inline ground / snap / collision math out of `usePartDrag`, `useGroupDrag`, `usePartResize`, `RotationHandle`, and `useKeyboardShortcuts` into per-tool pipelines that all share the same chain for preview and commit. 17 engine tests lock the constraint behaviors + pipeline composition + cross-constraint ordering.
