...
- **Phase §3 Interaction Session Controller is complete** ([ADR-003](./adr/003-session-controller.md)): a pure state machine (`sessionController.ts`) plus a React hook (`useCanvasPointerSession.ts`) own the canvas pointer-event surface. The state machine emits typed semantic actions — click, doubleclick, contextmenu, dragstart/move/commit/cancel — to the host. Workspace.tsx lost ~240 LOC: two large native-listener `useEffect` blocks (`mousedown`/`mouseup`/`dblclick`/`blur` and the window `contextmenu` listener), the `leftClickDownPos` / `rightClickDownPos` / `previousSelectionKeyRef` cross-handler refs, and the bookkeeping useEffect that tracked selection-key changes. The 24-case engine test suite locks in the additive-selection double-toggle, overlay-portal contextmenu, and drag-then-quick-click regression classes.
- **Phase §4 Tool Solver foundation and first hook migrations are complete** ([ADR-004](./adr/004-tool-solver-interface.md)): `ToolSolver<Input, State, Preview>` interface plus four tools (`moveTool`, `groupMoveTool`, `resizeTool`, `rotationTool`) around the shared transform math. Tools own preview/commit math; hooks own pointer-event plumbing. Commit instructions (`updatePartPosition` / `updatePartRotation` / `updatePartDimensions` / `updateGroupPositions`) flow through the `applyCommitInstructions` helper so tool commit paths produce predictable history writes. Focused contract tests lock in the begin/update/commit/cancel lifecycle and the preview=commit invariant per tool. `usePartResize`, `usePartDrag`, and `useGroupDrag` now delegate their preview solver blocks to tools; part/keyboard/store rotation paths delegate rotation math to `rotationTool`. Remaining §4 work is deeper cleanup: reduce the hook scaffolding around pointer projection, release handling, and batch/group commit plumbing.
- **Phase §10 Overlay Model is complete** ([ADR-005](./adr/005-overlay-model-derivation.md)): a single pure function `computeOverlayModel` derives every overlay's display data into a typed `OverlayModel` (`snap` / `references` / `dimensions` slots). All three overlay components — `SnapAlignmentLines`, `MultiSelectionDimensions`, `ReferenceDistanceIndicators` — are pure prop consumers; they no longer read state from stores directly. (Action references are still read from `projectStore` inside `ReferenceDistanceIndicators` for the inline edit submit handler, since those are stable function references and don't subscribe to state.) Workspace subscribes once and passes each model slice down. 13 derivation tests lock the slot rules + purity.
- **Phase §1a Scene Graph foundation is complete** ([ADR-008](./adr/008-scene-graph-derivation.md)): a memoized `WorkspaceSceneGraph` adapter over `projectStore.parts / groups / groupMembers`. Domain truth still lives in the project store — the adapter is derived, not persisted. `buildWorkspaceSceneGraph` produces a typed node tree with O(1) `findNode`, memoized `descendantPartIds` and `ancestorGroupIds`, defensive duplicate-id / cycle detection. 23 engine tests cover flat/single-level/nested/disjoint/malformed cases plus the S0–S5 fixtures. Phase §1b-1 migrated the first consumer (`PartsRenderer`) to use the adapter via `useWorkspaceSceneGraph`. Remaining `getAllDescendantPartIds` consumers (clipboardStore, HierarchicalPartsList, PropertiesPanel, useKeyboardShortcuts, projectStore internals) migrate incrementally; §1c retires the legacy traversal helper.
- **Phase §5 Geometry Bundle foundation and first consumer migrations are complete** ([ADR-009](./adr/009-geometry-bundle-cache.md)): a typed `PartGeometryBundle` per part with bounds (AABB + OBB), render mesh descriptor, hit proxy, snap anchor graph (faces / edges / corners / fraction lines), measurement graph, and collision proxy. `deriveBoxBundle` handles the current box-shaped `Part`; a read-through `GeometryCache` invalidates on dimension or rotation change and returns identical references when version keys match. Engine tests cover the version-key rules + per-field bundle correctness + cache lifecycle. Snap/bounds parity tests cover `getPartBounds` / `getPartOBB`; hit-testing, measurement AABB, ground constraints, and collision constraints now all have cache-backed seams. When §6 (Part Shape Model) adds fabrication operations to `Part`, the dispatcher inside `deriveBundle` grows without consumers needing to change.
- **Phase §8 Constraint Pipeline foundation plus move/resize/rotate ground and move-collision wiring are complete** ([ADR-006](./adr/006-constraint-pipeline.md)): a typed `Constraint` interface, an `applyConstraints` pipeline runner, and the `groundConstraint`, `stockDimensionConstraint`, and `collisionConstraint` pipeline stages. Constraints take a `CandidateTransform` (move / resize / rotate variants), produce an adjusted transform plus blockers + warnings, and read bounds through the shared `GeometryCache` when geometry is needed. Focused engine tests lock the constraint behaviors + pipeline composition + cross-constraint ordering. The remaining §8 work is deeper hook collapse: shrinking `usePartDrag`, `useGroupDrag`, `usePartResize`, `RotationHandle`, and keyboard handlers so they delegate more fully to exported per-tool pipelines.
- **Workspace right-click target globals deleted**: the `globalRightClickTarget` bridge in `workspaceUtils` (used by per-mesh R3F pointerdown handlers to communicate with the workspace `contextmenu` listener) is gone. The session controller's `onContextMenu` resolves right-click targets directly via the hit-test service. Net: -114 LOC including the test file's right-click-target block.
