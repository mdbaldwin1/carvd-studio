...
- **Phase §4 Tool Solver foundation is complete** ([ADR-004](./adr/004-tool-solver-interface.md)): `ToolSolver<Input, State, Preview>` interface plus three wrappers (`moveTool`, `groupMoveTool`, `resizeTool`) around the existing pure solvers. Tools own the math; hooks own the pointer-event plumbing. Commit instructions (`updatePartPosition` / `updatePartRotation` / `updatePartDimensions` / `updateGroupPositions`) flow through the `applyCommitInstructions` helper so every tool's commit path produces a predictable single history entry. 23 contract tests lock in the begin/update/commit/cancel lifecycle and the preview=commit invariant per tool. Phase §4b will refactor `usePartDrag` / `useGroupDrag` / `usePartResize` to delegate to these tools (~1500 LOC → ~500 LOC across the hooks), and extract the rotation math from `RotationHandle` into a fourth tool.
