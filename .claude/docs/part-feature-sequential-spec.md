...
- Within that queue, content belongs to each request but the destination belongs to the document generation. Ordinary/autosave/close/Quit saves resolve the last successful preceding Save As destination when they execute, including saves queued before its chooser resolves. Only a successful Save As advances that destination; cancellation/failure retains the previous path, or lets a later initial save ask for a path when none exists. Filename-derived automatic renames advance with the same transaction; queued snapshots inherit them only when their captured name still matches the prior automatic name. Authored name edits are retained, and a failed Save As does not rename the live project. A later explicit Save As is a new destination choice; New/Open receives an independent context. Queue state is runtime-only, bounded to pending work, and does not change the file schema or undo history.
