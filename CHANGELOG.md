...
- **Desktop tutorial component migration (bead carvd-studio-7.5)** — Migrated tutorial flows to shadcn primitives by wrapping `WelcomeTutorial` in shadcn `Dialog` infrastructure, replacing tutorial tooltip structure with shadcn `Card`/`Progress`/`Button` composition, and preserving spotlight overlay positioning plus keyboard navigation behavior.
- **Desktop StartScreen Card/navigation migration (bead carvd-studio-7.6)** — Completed StartScreen layout migration by converting remaining StartScreen actions and template navigation controls to shadcn `Button`, migrating template preview tiles in `TemplatesSection` to shadcn `Card` interactions, and updating StartScreen/template tests to role-based card selectors.
