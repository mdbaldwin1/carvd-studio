...
- **Preview-handle editing in Part Cuts (POC)** — Added direct preview handles for supported face-based operations so cutouts, mortises, stopped dados, and stopped grooves can be moved or resized from the cuts preview while the inspector remains the exact-entry fallback.
...
- **Feature visibility across lists and assemblies (POC)** — Parts lists and assembly part previews now show concise `Ops` badges and woodworking-oriented operation summaries so authored mitres/notches/cutouts remain visible outside the Properties panel.
- **Feature-aware cut lists and exports (POC)** — Cut-list validation now checks saved operation constraints, parts/CSV/PDF reports now show blank-first dimensions plus operation instructions, and cutting diagrams explicitly communicate that feature operations happen after blank breakdown.
