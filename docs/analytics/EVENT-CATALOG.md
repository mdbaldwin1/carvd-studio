...
Desktop owner: Electron main analytics service; renderer capture sites own trigger accuracy. Events are accepted only after explicit consent and sanitized against the exact schema below.
...
| `analytics_consent_changed` | A persisted grant from onboarding or Privacy settings                | `choice=granted`; `surface=onboarding\|settings`                                               | Consent rollout health only                      | `event=analytics_consent_changed`, breakdown `surface`; denial must emit nothing |
...
Desktop-specific prohibited examples: project UUID, counts outside buckets, raw error messages, optimizer inputs, material/library contents, machine/user name, full OS paths, license response, or activation email.
...
| `download_clicked` | Installer link click before normal navigation      | `platform=macos\|windows`; `location=home-hero-card\|download-hero-card\|download-cta-footer`     | Installer intent and acquisition funnel | `event=download_clicked`, breakdown `platform`, `location`                    |
