...
function addWatermark(doc: jsPDF, pageWidth: number, pageHeight: number): void {
  ...
  doc.text('Generated by Carvd Studio - carvd-studio.com', pageWidth / 2, pageHeight - 15, { align: 'center' });
}
...
export async function exportProjectReportToPdf(
  ...
  for (const [, boards] of boardsByStock) {
    ...
    for (const board of boards) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(0, 0, 0);
      doc.text(`${stockName} - Board #${board.boardIndex}`, lmargin, lmargin + 20);

      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      const dims = `${formatMeasurementWithUnit(board.stockLength, units)} x ${formatMeasurementWithUnit(board.stockWidth, units)}`;
      doc.text(dims, lmargin, lmargin + 38);

      doc.setFontSize(10);
      doc.text(
        `${board.placements.length} part${board.placements.length !== 1 ? 's' : ''} | ${board.utilizationPercent.toFixed(1)}% utilization`,
        lpageWidth - lmargin - 150,
        lmargin + 20
      );

      // Diagram area
      const headerHeight = 50;
      const diagramAreaTop = lmargin + headerHeight;
      const diagramAreaHeight = lpageHeight - diagramAreaTop - lmargin - 30;
      const diagramAreaWidth = lcontentWidth;
      ...
      const diagramX = lmargin + (diagramAreaWidth - diagramWidth) / 2;
      ...
      for (const placement of board.placements) {
        ...
        const py = diagramY + placement.y * scale;
        ...
        const labelDims = `${formatMeasurementWithUnit(placement.width, units)} x ${formatMeasurementWithUnit(placement.height, units)}`;
      ...
      doc.setTextColor(0, 0, 0);
      addWatermark(doc, lpageWidth, lpageHeight);
    }
  }
