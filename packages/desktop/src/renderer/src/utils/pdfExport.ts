...
import { CutList, StockBoard, CustomShoppingItem } from '../types';
import { getInstructionFabricationSummary, groupCutInstructions } from './cutListInstructions';
...
export async function exportCutListToPdf(
  ...
  doc.text('Blank sizes shown below. Apply operations after rough cutting blanks to size.', margin, margin + 68);
  ...
  // Table header
  let y = margin + 94;
  const colWidths = [12, 28, 138, 56, 56, 44, 74, 96];
  const colX = [
    margin + 2,
    margin + 18,
    margin + 48,
  ...
  for (const group of grouped) {
    ...
    let displayOps = [partBoards ? `Boards ${partBoards}` : '', fabricationSummary].filter(Boolean).join(' | ') || '-';
    while (doc.getTextWidth(displayOps) > colWidths[7] - 4 && displayOps.length > 6) {
      displayOps = displayOps.substring(0, displayOps.length - 4) + '...';
    }
...
export async function exportProjectReportToPdf(
  ...
  // Table header
  const colWidths2 = [12, 24, 126, 58, 58, 44, 74, 102];
  const colX2 = [
    margin + 2,
    margin + 16,
    margin + 44,
  ...
  for (const group of grouped) {
    ...
    let displayOps = [partBoards ? `Boards ${partBoards}` : '', fabricationSummary].filter(Boolean).join(' | ') || '-';
    while (doc.getTextWidth(displayOps) > colWidths2[7] - 4 && displayOps.length > 6) {
      displayOps = displayOps.substring(0, displayOps.length - 4) + '...';
    }
  ...
  for (const [, boards] of boardsByStock) {
    for (const board of boards) {
      ...
      doc.setTextColor(90, 90, 90);
      doc.text(
        'Diagram shows blank layout only. Apply mitres, notches, and cutouts after blank breakdown.',
        diagramMargin,
        dpageHeight - 28
      );
