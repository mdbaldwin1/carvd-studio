...
describe("FeaturesPage", () => {
  describe("rendering", () => {
    ...
    it("renders page subtitle", () => {
      ...
      expect(
        screen.getByText(
          /Designed by woodworkers who were tired of complicated software/i,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("main features", () => {
    it("renders 3D design section", () => {
      renderFeaturesPage();
      expect(
        screen.getByText(/3D Design That Makes Sense/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Real-Time 3D Visualization/i),
      ).toBeInTheDocument();
      expect(screen.getByText(/Intuitive Interface/i)).toBeInTheDocument();
      expect(
        screen.getByText(/Catch Mistakes Before Cutting/i),
      ).toBeInTheDocument();
    });
    ...
    it("renders cut list optimization section", () => {
      renderFeaturesPage();
      expect(
        screen.getByText(/Smart Cut List Generation/i),
      ).toBeInTheDocument();
    ...
    it("renders privacy section", () => {
      ...
      expect(
        screen.getByText(/Offline-First Project Editing/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          /internet is used for license activation, updates, and optional analytics/i,
        ),
      ).toBeInTheDocument();
      expect(
  ...
  describe("additional features", () => {
    ...
    it("renders lightning fast performance card", () => {
      renderFeaturesPage();
      expect(
        screen.getByText(/Lightning Fast Performance/i),
      ).toBeInTheDocument();
    });
