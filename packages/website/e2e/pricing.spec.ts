...
test.describe('Pricing Page', () => {
  ...
  test.describe('Hero Section', () => {
    ...
    test('displays value proposition badge', async ({ page }) => {
      await expect(page.locator('.badge').first()).toContainText('Less Than 6 Months');
    });
  });
  ...
  test.describe('Pricing Card', () => {
    ...
    test('displays feature checklist', async ({ page }) => {
      // The main pricing card checklist has 12 items (there are other checklists on the page)
      const pricingCardChecklist = page.locator('.card .checklist').first();
      await expect(pricingCardChecklist.locator('li')).toHaveCount(12);
    });

    test('displays key features in checklist', async ({ page }) => {
      await expect(page.locator('text=Full 3D furniture design studio')).toBeVisible();
      await expect(page.locator('text=Intelligent cut list optimizer')).toBeVisible();
      await expect(page.locator('text=Free lifetime updates')).toBeVisible();
      await expect(page.locator('text=Install on up to 3 devices')).toBeVisible();
    });
    ...
    test('displays trust signals', async ({ page }) => {
      await expect(page.locator('text=30-day money-back guarantee').first()).toBeVisible();
  ...
  test.describe('Value Comparison Section', () => {
    test('displays break-even heading', async ({ page }) => {
      await expect(page.locator('text=The Break-Even Point? 6 Months.')).toBeVisible();
    });

    test('displays subscription vs one-time comparison', async ({ page }) => {
      await expect(page.locator('text=Monthly Subscription').first()).toBeVisible();
  ...
  test.describe('ROI Calculator Section', () => {
    test('displays ROI heading', async ({ page }) => {
      await expect(page.locator('text=How It Could Pay For Itself')).toBeVisible();
    });

    test('displays hypothetical example disclaimer', async ({ page }) => {
      // "hypothetical example" appears twice - use first()
      await expect(page.locator('text=hypothetical example').first()).toBeVisible();
    });
  ...
  test.describe('Competitor Comparison Section', () => {
    test('displays comparison heading', async ({ page }) => {
      await expect(page.locator('text=How We Compare to Other Software')).toBeVisible();
    });
  ...
  test.describe('FAQ Section', () => {
    ...
    test('displays FAQ questions', async ({ page }) => {
      await expect(page.locator('text=Is this really a one-time payment?')).toBeVisible();
      await expect(page.locator('text=Do I get future updates?')).toBeVisible();
      await expect(page.locator("text=What if I'm not satisfied?")).toBeVisible();
      await expect(page.locator('text=Can I use it on multiple computers?')).toBeVisible();
